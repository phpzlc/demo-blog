<template>
    <script v-bind:id="id" type="text/plain" style="width:100%;height:500px;" v-html="content"></script>
</template>
...
<script>
    ...
    UE.Editor.prototype.getActionUrl = function (action) {
        ...
        if(globalApiUrl.indexOf('?') != -1){
            fh = '&';
        }

        /* 按config中的xxxActionName返回对应的接口地址 */
        if (action == 'uploadimage') {
            return globalApiUrl + fh + 'uploadType=1';
        } else if (action == 'uploadscrawl') {
            return globalApiUrl + fh + 'uploadType=1';
        } else if (action == 'uploadvideo') {
            return globalApiUrl + fh + 'uploadType=2'
        } else if (action == 'uploadfile') {
            return globalApiUrl + fh + 'uploadType=4'
        } else {
    ...
    window.UEDITOR_CONFIG['imageUploadService'] = function (context, editor) {
        return {
             ...
             * */
            // imageSrcField:  'data.pathUrl'
            imageSrcField: 'data.server_path'
        }
    };
    ...
    window.UEDITOR_CONFIG['videoUploadService'] = function (context, editor) {
        return {
             ...
             * */
            videoSrcField: 'data.server_path'
        }
    };
    ...
    window.UEDITOR_CONFIG['scrawlUploadService'] = function (context, editor) {
        return scrawlUploadService = {
             ...
             * res.videoSrcField =  'data.pathUrl';
             */
